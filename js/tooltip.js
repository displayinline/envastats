...
;(function($, window, document, undefined)
{
	...
	/**
	 * Internal reference: the div holding tooltips
	 * @var jQuery
	 */
	var _tooltipsDiv = false;

	/**
	 * Internal function: retrieve the div holding tooltips
	 *
	 * @return jQuery the div selection
	 */
	function _getTooltipsDiv()
	{
		if (!_tooltipsDiv)
		{
			_tooltipsDiv = $('<div id="envastats-tooltips" class="envastats-scope"></div>').appendTo(document.body);
		}
	 ...
	 */
	$.fn.envastatsTooltip = function(content, options)
	{
		// Settings
		var globalSettings = $.extend({}, $.fn.envastatsTooltip.defaults, options),
		...
		this.each(function(i)
		{
			...
			if (!$.isReady || (!options.ignoreDelay && (options.delay > 0 || inlineOptions.delay > 0)))
			{
				...
				{
					abort(false, true);
					target.envastatsTooltip(content, $.extend(thisOptions, { ignoreDelay: true }));
			...
			else
			{
				...
				var previous = target.data('tooltip'),
					...
					// Functions
					updatePosition, removeEnvastatsTooltip, endRemove;
				...
				if (previous)
				{
					...
					// Remove
					if (previous.removeEnvastatsTooltip(false, true) === false)
					{
						return;
					}
				...
				{
					optionHolder = target.closest('.envastats-tooltip-top, .envastats-tooltip-right, .envastats-tooltip-bottom, .envastats-tooltip-left');
					if (optionHolder.length > 0)
					{
						awaitingOptions.position = /envastats-tooltip-(top|right|bottom|left)/.exec(optionHolder[0].className)[1];
					}
				}
				...
				if (settings.ajax && !settings.fromAjax)
				{
					...
					promise.done(function(data)
					{
						...
						if (current)
						{
							target.envastatsTooltip(data, settings);
						}
					});
					...
					promise.fail(function()
					{
						...
						if (current)
						{
							target.envastatsTooltip(settings.ajaxErrorMessage, settings);
						}
					});
				}
				...
				if ( settings.local )
				{
					...
					{
						parent.addClass( 'envastats-relative' );
					}
				}
				else
				{
					parent = _getTooltipsDiv();
				}
				...
				if (settings.exclusive && exclusive)
				{
					...
					if (exclusive.removeEnvastatsTooltip(false, true) === false)
					{
						return;
					}
				}
				...
				tooltip = $('<div class="envastats-tooltip '+settings.classes.join(' ')+noPointerEvents+'">'+content+'</div>')
							.appendTo(parent)
				...
				{
					case 'right':
						arrow = $('<span class="envastats-arrow left"><span></span></span>').appendTo(tooltip);
						arrowOffset = arrow.parseCSSValue('margin-top');
						break;

					case 'bottom':
						arrow = $('<span class="envastats-arrow top"><span></span></span>').appendTo(tooltip);
						arrowOffset = arrow.parseCSSValue('margin-left');
						break;

					case 'left':
						arrow = $('<span class="envastats-arrow right"><span></span></span>').appendTo(tooltip);
						arrowOffset = arrow.parseCSSValue('margin-top');
						break;

					default:
						arrow = $('<span class="envastats-arrow"><span></span></span>').appendTo(tooltip);
						arrowOffset = arrow.parseCSSValue('margin-left');
						break;
				}
				...
				// Remove
				removeEnvastatsTooltip = function(force, skipAnimation)
				{
					// Callback
				...
				target.data('tooltip', {
					...
					removeEnvastatsTooltip: removeEnvastatsTooltip
				});
	 ...
	 */
	$.fn.removeEnvastatsTooltip = function(force, skipAnimation)
	{
		this.each(function(i)
		{
			var target = $(this),
			...
			{
				// Remove
				if (tooltip.removeEnvastatsTooltip(force, skipAnimation) === false)
				{
					return;
				}
			}
	 ...
	 */
	$.fn.envastatsMenuTooltip = function(content, options, eventName)
	{
		// Parameters
		...
		this.on(eventName, function(event)
		{
			...
			// Open menu
			$(this).envastatsTooltip(content, $.extend({
	...
	doc.on('mouseenter', '.with-envastats-tooltip, .children-envastats-tooltip > *', function(event)
	{
		var element = $(this),
		...
		if (parent.hasClass('children-envastats-tooltip'))
		{
			options = $.extend(options, parent.data('tooltip-options'));
		}

		// Show tooltip
		element.envastatsTooltip(options);
	...
	win.on( 'resize', function()
	{
		...
		if ( _tooltipsDiv )
		{
			layers = layers.add( _tooltipsDiv );
		}
		...
		if ( layers.length > 0 )
		{
			...
			{
				var tooltip = $( this ),
					target = tooltip.data( 'tooltip-target' ),
					data;
